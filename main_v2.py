...
if __name__ == "__main__":
    sizes = [100, 200, 400, 800]  # Matrix sizes to test
    ...
    for num_processes in num_processes_list:
        ...
        plt.plot(sizes, speedups, label=f"Speedup ({num_processes} processes)")
    plt.xlabel("Matrix Size")
    plt.ylabel("Speedup")
    plt.legend()
    plt.title("Speedup with Multiprocessing")
    plt.show()
    ...
    for num_processes in num_processes_list:
        speedups = [times["sequential"][i] / times[num_processes][i] for i in range(len(sizes))]
        efficiencies = [speedup / num_processes for speedup in speedups]
        plt.plot(sizes, efficiencies, label=f"Efficiency ({num_processes} processes)")
    plt.xlabel("Matrix Size")
    plt.ylabel("Efficiency")
    plt.legend()
    plt.title("Efficiency with Multiprocessing")
    plt.show()

    # Plot memory complexity for each method and number of processes
    plt.figure()
    ...
    plt.legend()
    plt.title("Memory Complexity for Sequential and Multiprocessing")
    plt.show()
