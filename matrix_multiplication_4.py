...
def run_tests(matrix_sizes, num_nodes):
    ...
    for matrix_size in matrix_sizes:
        if rank == 0:
            ...
            print(f"Matrix size: {matrix_size}x{matrix_size} - Sequential CPU: {time.time() - start_time} seconds")

            # Sekwencyjne GPU
            start_time = time.time()
            C_gpu = gpu_matrix_multiply_sequential(A, B)
            print(f"Matrix size: {matrix_size}x{matrix_size} - Sequential GPU: {time.time() - start_time} seconds")

        # Synchronizacja procesów przed kolejnymi testami
        comm.Barrier()
